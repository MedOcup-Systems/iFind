...
# CORRIGIDO: função expandida + chamada descomentada
# ---------------------------------------------------------------------------
...
def _tesseract_ok() -> bool:
    ...
    1. PATH do sistema
    2. Caminhos padrão do Windows (Program Files + AppData)
    3. Pasta portátil local (tesseract_bin/ criada pelo setup.py sem admin)
    4. config_tesseract.py gerado pelo instalador
    Se encontrar por qualquer caminho, atualiza o PATH da sessão imediatamente.
    """
    # 1. PATH normal
    if shutil.which("tesseract"):
        return True

    # 2. Caminhos padrão Windows
    ...
    # 3. Pasta portátil local do projeto (instalação sem admin)
    ...
    # 4. config_tesseract.py aponta para um exe válido
...
def _mostrar_instrucoes_manuais():
    """Exibe instruções de instalação manual conforme o SO detectado."""
...
def _verificar_tesseract():
    ...
    Se o Tesseract não for encontrado, executa o setup.py com 4 estratégias
    em cascata e exibe o progresso na interface Streamlit.
    Após sucesso, chama st.rerun() para recarregar o app automaticamente.
    """
    if _tesseract_ok():
        return  # tudo certo, continua normalmente
    ...
    # Localiza o instalador — aceita tanto setup_tesseract.py quanto setup.py
    ...
    # Usa elementos simples em vez de st.status() para compatibilidade
    # com todas as versões do Streamlit (st.status pode retornar None < v1.28)
    ...
    if platform.system().lower() == "windows" and not _tem_admin():
        # Lança o setup via ShellExecute com "runas" para obter privilégios UAC
        # subprocess.run() herda as permissões do Streamlit (sem admin) e falharia
        ...
        if res <= 32:
            ...
            st.stop()
        # O script está rodando elevado em outro processo
        # Aguarda e verifica se o Tesseract apareceu
        import time
        st.info("Aguardando a instalação concluir (até 3 minutos)...")
        for _ in range(36):  # tenta por até 3 minutos
            time.sleep(5)
            if _tesseract_ok():
                break
        # Cai na verificação abaixo
    else:
        # Já tem admin ou é Linux/Mac — roda normalmente via subprocess
...
# ---------------------------------------------------------------------------
# CHAMADA DA VERIFICAÇÃO — executa sempre que o app carrega
# ---------------------------------------------------------------------------
...
def _main():
    ...
    Isolar em função garante que st.stop() no tela_login() funcione corretamente
    — o Streamlit interrompe apenas o escopo da função, não o módulo inteiro.
    """
    inicializar_banco()
    tela_login()
    ...
    if usuario is None:
        return  # fallback: tela_login() não bloqueou
    ...
    with aba_busca:
        ...
        with col1:
            ...
            # Campo drive_raiz + botão seletor de pasta nativo
            ...
            with _dr_col2:
                ...
                if st.button("🗂️", key="btn_drive", help="Selecionar pasta pelo explorador"):
        ...
        with col2:
            # Campo pasta_destino + botão seletor de pasta nativo
            ...
            with _pd_col2:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("🗂️", key="btn_destino", help="Selecionar pasta pelo explorador"):
                    from processor import abrir_seletor_pasta
    ...
    with aba_config:
        ...
        with st.expander("Alterar senha"):
            ...
            if st.button("Salvar senha"):
                ...
                # Usa o campo "usuario" (login) para verificar, não o nome
