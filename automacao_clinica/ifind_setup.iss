...
[Icons]
; Area de Trabalho
Name: "{userdesktop}\{#AppName}"; Filename: "{code:GetPythonW}"; Parameters: """{app}\{#LauncherPy}"""; WorkingDir: "{app}"; IconFilename: "{app}\ifind.ico"; Comment: "Abrir iFind Clinica"; Tasks: desktopicon
; Menu Iniciar - atalho principal
Name: "{userprograms}\{#AppName}\{#AppName}"; Filename: "{code:GetPythonW}"; Parameters: """{app}\{#LauncherPy}"""; WorkingDir: "{app}"; IconFilename: "{app}\ifind.ico"; Comment: "Abrir iFind Clinica"; Tasks: startmenu
; Menu Iniciar - terminal (debug)
Name: "{userprograms}\{#AppName}\{#AppName} (modo terminal)"; Filename: "{app}\iniciar.bat"; WorkingDir: "{app}"; IconFilename: "{app}\ifind.ico"; Comment: "Abrir com terminal visivel"; Tasks: startmenu
...
; ================================================================
[Run]
Filename: "{code:GetPythonW}"; Parameters: """{app}\{#LauncherPy}"""; WorkingDir: "{app}"; Description: "Iniciar {#AppName} agora"; Flags: nowait postinstall skipifsilent
...
// ── Verifica se um caminho e valido e NAO e do Windows Store ──────
// Python da Microsoft Store fica em WindowsApps e retorna
// "Acesso negado; codigo 5" quando qualquer outro processo tenta
// executa-lo via CreateProcess — incluindo o Inno Setup.
// Tambem rejeita os stubs em Microsoft\WindowsApps.
function CaminhoValido(Caminho: String): Boolean;
begin
  Result := FileExists(Caminho)
    and (Pos('WindowsApps', Caminho) = 0)
    and (Pos('windowsapps', LowerCase(Caminho)) = 0);
end;
...
var
  Vers: TArrayOfString;
  i: Integer;
  TestPath, RegBase: String;
  Candidatos: TArrayOfString;
begin
  Result := False;
  PythonWPath := '';

  Vers := ['3.13', '3.12', '3.11', '3.10', '3.9'];

  // ── 1. Registro HKCU (instalacao do usuario atual, sem admin) ──
  // Python.org instala aqui por padrao quando InstallAllUsers=0
  ...
  begin
    ...
    begin
      ...
      if CaminhoValido(TestPath) then
      begin
  ...
  // ── 2. Registro HKLM (instalacao global, com admin) ──────────
  for i := 0 to GetArrayLength(Vers) - 1 do
  begin
    RegBase := 'Software\Python\PythonCore\' + Vers[i] + '\InstallPath';
  ...
  // ── 3. Caminhos fixos do instalador python.org ────────────────
  // Ordem: mais nova para mais antiga, usuario local antes de global
  SetArrayLength(Candidatos, 16);
  Candidatos[0]  := ExpandConstant('{localappdata}\Programs\Python\Python313\pythonw.exe');
  Candidatos[1]  := ExpandConstant('{localappdata}\Programs\Python\Python312\pythonw.exe');
  Candidatos[2]  := ExpandConstant('{localappdata}\Programs\Python\Python311\pythonw.exe');
  Candidatos[3]  := ExpandConstant('{localappdata}\Programs\Python\Python310\pythonw.exe');
  Candidatos[4]  := ExpandConstant('{localappdata}\Programs\Python\Python39\pythonw.exe');
  Candidatos[5]  := 'C:\Python313\pythonw.exe';
  Candidatos[6]  := 'C:\Python312\pythonw.exe';
  Candidatos[7]  := 'C:\Python311\pythonw.exe';
  Candidatos[8]  := 'C:\Python310\pythonw.exe';
  Candidatos[9]  := 'C:\Python39\pythonw.exe';
  Candidatos[10] := ExpandConstant('{autopf64}\Python313\pythonw.exe');
  Candidatos[11] := ExpandConstant('{autopf64}\Python312\pythonw.exe');
  Candidatos[12] := ExpandConstant('{autopf64}\Python311\pythonw.exe');
  Candidatos[13] := ExpandConstant('{autopf}\Python313\pythonw.exe');
  Candidatos[14] := ExpandConstant('{autopf}\Python312\pythonw.exe');
  Candidatos[15] := ExpandConstant('{autopf}\Python311\pythonw.exe');

  for i := 0 to GetArrayLength(Candidatos) - 1 do
  begin
    if CaminhoValido(Candidatos[i]) then
    begin
...
begin
  ...
  // 1. Tenta via BuscarPythonW — ja filtra WindowsApps automaticamente
  ...
  begin
    ...
    begin
      // Garante que o pip tambem existe no mesmo Python
      PipePath := ExtractFilePath(PythonWPath) + 'Scripts\pip.exe';
  ...
  // 2. Fallback via cmd — mas verifica se nao e WindowsApps
  // O Python Store nao retorna erro no "where python" mas falha no Exec
  // Entao verificamos explicitamente se o caminho retornado e valido
  Result := False;
end;
...
begin
  ...
  MsgBox(
    'Python compativel nao foi encontrado neste computador.' + #13#10 + #13#10 +
    'ATENCAO: Python da Microsoft Store nao e suportado.' + #13#10 +
    'Se voce tem o Python da Store, ele sera ignorado.' + #13#10 + #13#10 +
    'O instalador ira baixar o Python 3.11 oficial (python.org)' + #13#10 +
    'automaticamente. Isso requer internet e pode levar alguns minutos.' + #13#10 + #13#10 +
